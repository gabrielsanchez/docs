...
This tutorial demonstrates the core W&B features using a simple MNIST classification example, showing you how to log hyperparameters, monitor training progress, and save models.
...
1. **Initialize a run** with `wandb.init()` to start tracking an experiment.

2. **Store hyperparameters** in `run.config` to keep a record of the settings used for each run.
...
To get started, you'll need to get the library.
